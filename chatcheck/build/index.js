...
// API 调用辅助函数
async function fetchSupportChats(filter = "pending", page = 1, limit = 50) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to fetch support chats: ${response.status} ${response.statusText} - ${errorText}`);
    }
...
async function sendSupportReply(conversationId, content) {
        ...
        tools: [
            {
                ...
                description: "获取待处理的 support chat 列表。返回需要回复的用户对话，包括用户信息、最后一条消息和最近的对话历史。",
                inputSchema: {
                    type: "object",
                    properties: {
                        limit: {
            ...
            },
            {
                ...
                description: "获取所有 support chat（包括已回复和待处理的）。可以用于查看完整的 support 对话列表。",
                inputSchema: {
                    type: "object",
                    properties: {
                        filter: {
                            ...
                            description: "过滤类型：'all' 所有对话，'pending' 仅待处理，'answered' 仅已回复。默认 'all'",
                        },
                        limit: {
                            type: "number",
                            description: "返回的最大对话数量，默认 50",
...
// 工具处理函数
async function handleGetPendingSupportChats(args) {
...
async function handleGetAllSupportChats(args) {
    const filter = args?.filter || "all";
    const limit = args?.limit || 50;
    const page = args?.page || 1;
    const data = await fetchSupportChats(filter, page, limit);
    return {
        content: [
            {
                type: "text",
...
async function handleGetConversationMessages(args) {
...
async function handleReplyToSupportChat(args) {
    const conversationId = args?.conversationId;
